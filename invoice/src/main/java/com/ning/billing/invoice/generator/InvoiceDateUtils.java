...
public class InvoiceDateUtils {
    ...
    public static LocalDate calculateEffectiveEndDate(final LocalDate billCycleDate, final LocalDate targetDate,
                                                      final BillingPeriod billingPeriod, final int billingCycleDayLocal) {
        if (targetDate.isBefore(billCycleDate)) {
            return billCycleDate;
        }

        return calculateProposedEndBillCycleDateAfter(billCycleDate, targetDate, billingPeriod, billingCycleDayLocal);
    }

    public static LocalDate calculateEffectiveEndDate(final LocalDate billCycleDate, final LocalDate targetDate,
                                                      final LocalDate endDate, final BillingPeriod billingPeriod, final int billingCycleDayLocal) {
        if (targetDate.isBefore(endDate)) {
            if (targetDate.isBefore(billCycleDate)) {
                return billCycleDate;
            }

            final LocalDate proposedDate = calculateProposedEndBillCycleDateAfter(billCycleDate, targetDate, billingPeriod, billingCycleDayLocal);
    ...
    private static LocalDate calculateProposedEndBillCycleDateAfter(final LocalDate billCycleDate, final LocalDate targetDate, final BillingPeriod billingPeriod, final int billingCycleDayLocal) {
        ...
        proposedDate = alignProposedBillCycleDate(billingCycleDayLocal, proposedDate);
        return proposedDate;
    }

    public static LocalDate alignProposedBillCycleDate(final int billingCycleDayLocal, final LocalDate proposedDate) {

        final int lastDayOfMonth = proposedDate.dayOfMonth().getMaximumValue();

        int proposedBillCycleDate = proposedDate.getDayOfMonth();
        if (proposedDate.getDayOfMonth() < billingCycleDayLocal && billingCycleDayLocal <= lastDayOfMonth) {
            proposedBillCycleDate = billingCycleDayLocal;
        }
